...
// 문장 단위 자산 후보 목록은 여기 있었다. asset-requirements.mjs 의 kind:'sentence'
// 가 같은 일을 더 정확히 한다 — 미충족 기준 전량을 덮고 필요 자산까지 적으며
// check-asset-requirements 가 낡음을 잡는다. 같은 정보를 두 곳에 두면 한쪽이 낡는다.
...
export const PARTIAL_SCORING = {
  ...
  // 듣기·말하기 기준을 글 대화로 열 때는 partial 로 정직하게 표시한다 —
  // 글 대화 추론은 읽기 능력을 함께 재므로 듣기 그 자체가 아니다(5차 판정).
  '[6국01-01]': {
    scored: '글로 적힌 대화에서 생략된 내용을 추론하는 문항',
    notScored: '실제로 듣고 추론하며 말로 반응하는 수행',
  },
  '[6국01-02]': {
    scored: '글로 적힌 대화·주장에서 근거의 타당성을 판정하는 문항',
    notScored: '실제 담화를 들으며 판단하고 말로 참여하는 수행',
  },
  ...
  // '말하거나 쓰는/답하는' 기준의 말하기 절반은 사람이 본다(3차 판정의 부분 측정 원칙).
  '[4영02-06]': {
    scored: '상황에 알맞은 행동 지시 문장을 고르는 문항',
    notScored: '지시문을 실제로 말하는 수행',
  },
  '[4영02-08]': {
    scored: '물음에 알맞은 응답을 고르는 문항',
    notScored: '실제로 묻고 답하며 말하는 수행',
  },
  '[6영02-04]': {
    scored: '상황에 알맞은 소개·묘사 문장을 고르는 문항',
    notScored: '소개·묘사를 실제로 말하는 수행',
  },
  '[6영02-07]': {
    scored: '세부 정보를 묻는 말에 알맞은 응답을 고르는 문항',
    notScored: '실제로 묻고 답하며 말하는 수행',
  },
};
